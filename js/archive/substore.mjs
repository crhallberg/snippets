export function createStore(data = null, shouldNotify = () => true) {
    let subs = [];
...
export function createDerived(store, transform, shouldNotify = () => true) {
    const derived = createStore(transform(store.get()), shouldNotify);
    store.listen((op) => derived.set(transform(op)));
    return derived;
}

export function createLocalStore(key, initial = null, shouldNotify = () => true) {
